...
### Data visualization
data.files <- list.files('./MSMC2_Data/', pattern = 'EM500.MSMC2.out.final.txt')
data.combined <- list()
for ( file in 1:12){ #1:length(data.files)
  tmp <- read.table(paste0('./MSMC2_Data/',data.files[[file]]), header = T)
  if (file == 1) { tmp$pop <- c(rep('AM-1',nrow(tmp))); tmp$pop2 <- c(rep('AM',nrow(tmp)))}
  else if (file == 2) {tmp$pop <- c(rep('AM-2',nrow(tmp))); tmp$pop2 <- c(rep('AM',nrow(tmp)))}
  else if (file == 3) {tmp$pop <- c(rep('AM-3',nrow(tmp))); tmp$pop2 <- c(rep('AM',nrow(tmp)))}
  else if (file == 4) {tmp$pop <- c(rep('ATL-1',nrow(tmp))); tmp$pop2 <- c(rep('ATL',nrow(tmp)))}
  else if (file == 5) {tmp$pop <- c(rep('ATL-2',nrow(tmp))); tmp$pop2 <- c(rep('ATL',nrow(tmp)))}
  else if (file == 6) {tmp$pop <- c(rep('ATL-3',nrow(tmp))); tmp$pop2 <- c(rep('ATL',nrow(tmp)))}
  else if (file == 7) {tmp$pop <- c(rep('CAM-1',nrow(tmp))); tmp$pop2 <- c(rep('CAM',nrow(tmp)))}
  else if (file == 8) {tmp$pop <- c(rep('CAM-2',nrow(tmp))); tmp$pop2 <- c(rep('CAM',nrow(tmp)))}
  else if (file == 9) {tmp$pop <- c(rep('CAM-3',nrow(tmp))); tmp$pop2 <- c(rep('CAM',nrow(tmp)))}
...
ggplot(data=subset(data.combined, data.combined$pop2 != 'AM')[which(subset(data.combined, data.combined$pop2 != 'AM')$time <= 2.5e07),], aes(x=(time), y=log10(popsize), fill=pop, col=pop))+
  ...
  #scale_color_manual(values = c("#723957","#723957","#723957","#4E87B8","#4E87B8","#4E87B8","#A2662C","#A2662C", "#A2662C","gray50","gray50","gray50"))+
  scale_color_manual(values = c("#723957","#4E87B8","#A2662C","#4E87B8","#4E87B8","#4E87B8","#A2662C","#A2662C", "#A2662C","gray50","gray50","gray50"))+
...
##############################
## WAM-CAM
wam.cam <- list.files('./MSMC2_Data/CrossPopulation/', pattern = 'combined_WAM-CAM')
wam.cam.combined <- list()
for ( file in 1:length(wam.cam)){
  tmp <- read.table(paste0('./MSMC2_Data/CrossPopulation/',wam.cam[[file]]), header = T)
...
## AM-ATL:
am.atl <- list.files('./MSMC2_Data/CrossPopulation/', pattern = 'combined_AM-ATL')
am.atl.combined <- list()
for ( file in 1:length(am.atl)){
  tmp <- read.table(paste0('./MSMC2_Data/CrossPopulation/',am.atl[[file]]), header = T)
  if (file == 1) { tmp$set <- c(rep('1',nrow(tmp)))}
  else if (file == 2) {tmp$set <- c(rep('2',nrow(tmp)))}
  else if (file == 3) {tmp$set <- c(rep('3',nrow(tmp)))}
